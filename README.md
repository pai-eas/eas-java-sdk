...
2.0.5 及以上版本增加了 queue service 客户端功能，支持多优先级异步队列服务，如果需要使用该功能，为避免依赖包版本冲突，用户还需自行添加如下两个依赖包，并修改这两个依赖包至合适版本：
...
|               | Pair<Long, String> put(byte[] data, long priority, Map<String, String> tags)                                     | 功能：将数据写入队列服务。<br /><br />参数：<br />[data]: byte[] 数据 ；<br />[priority]: 数据优先级，priority值越小，优先级越高；默认优先级为0，为最高优先级；<br />[tags]: 自定义参数；<br /><br />返回值：Pair<Long, String> 类型的 <数据index, 请求Id>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
